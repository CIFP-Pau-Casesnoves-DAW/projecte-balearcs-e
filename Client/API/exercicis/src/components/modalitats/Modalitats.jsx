...
export default function Modalitats(props) {
    ...
    const [columnes, setColumnes] = useState([
        ...
        {field: "nom_modalitat", headerName: "Modalitat", width: 200, sortable: true, filter: true},
    ]);

    useEffect(() => {
        descarregaModalitats();
    }, []);

    const descarregaModalitats = async () => {
        try {
            const response = await fetch('http://balearc.aurorakachau.com/public/api/modalitats', {
                method: 'GET',
                headers: {
            ...
            setModalitats(responseData.data);
        } catch (error) {
            console.log(error);
        }
        setLoading(false);
    ...
    if (loading) {
        ...
        return (
            ...
            <hr />
                <Row md={9}>
                    <Col>
                        <h2>Llista de <b>Modalitats</b></h2>
                    </Col>
                ...
                <div className="ag-theme-quartz" style={{ height: 550, width: '100%' }}>
                    ...
                    >  
                        ...
                        <AgGridColumn field="nom_modalitat" headerName="Modalitat" width={200} sortable={true} filter={true}></AgGridColumn>
                    </AgGridReact>
                </div>
            </>
        );
