...
class ControlaDadesAudios
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if ($request->header('Authorization')) { // Hem rebut el header d’autorització?
            ...
            $audio_id = $request->route('id');
            $puntinteres_id = Audios::where('id', $audio_id)->value('punt_interes_id');;
            $espai_id = PuntsInteres::where('id', $puntinteres_id)->value('espai_id');
